...
public class HandDistributor : MonoBehaviour {
    ...
    public void Initialize() {  
        ...
        _cardResourceService = GameService.GetService<CardResourceService>();

	}

    //  cards shouldnt be smaller then hand size 
    public void DistributeCards(GameObject[] cards) {
        Debug.Log($"CARDS DISTRIBUTING, cards length = {cards.Length}");
        for (int i = 0; i < cards.Length; ++i) {
            if(cards[i] == null) {
                Debug.Log($"Card #{i} is null");
            }
        }
        ...
        //  distribute cards
        for (int i = 0; i < cards.Length; ++i){  //  creating card and placing it
            Debug.Log($"Cards distributing step #{i}");
            
            // ????????????????
            cards[i].transform.SetParent(transform, false);
            _cardResourceService.LoadCardsResource(cards[i]);
            ...
            if(i > _fullHandSize){
                ...
                Debug.Log("Too much cards in the hand");
    ...
    private CardResourceService _cardResourceService;
    //
