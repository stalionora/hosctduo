...
public class HandDistributor : MonoBehaviour {
    ...
    public void Initialize() {  
        ...
        //  -s- ... -c- o -c- o -c- ... -s-     -->     ... -c- o -c- ...
        _overflownHandSize = (int)((_activeAreaSize - _hand.LeftShift * 2) / (_hand.CardSize.CardScale.x  / 2));
        _fullHandSize = (int)((_activeAreaSize - _hand.LeftShift * 2 + _hand.OffsetBetweenCards) / (_hand.CardSize.CardScale.x + _hand.OffsetBetweenCards));  // 
        if (_hand.CardSize.CardScale.x / 2 * _hand.HandSize + _hand.LeftShift * 2 > _activeAreaSize)    // interrupt, if number of cards is greater than length of area / half card width
            Debug.Log("Больше карт чем можно вместить в руку");

        //
        _distanceBetweenCentersOfCards = _hand.CardSize.CardScale.x + _hand.OffsetBetweenCards;
    ...
    public void DistributeCards(GameObject[] cards) {
        if (cards.Length < _hand.HandSize){
        ...
        //  distribute cards
        for (int i = 0; i < _hand.HandSize; ++i)  //  creating card and placing it
        {
            // ????????????????
            cards[i].transform.SetParent(transform, false);
            cards[i].name = $"Card#{i}";
            Replace(cards, _midCoordinateOfTheHand.x - ((i + 1) * _hand.CardSize.CardScale.x + (i * _hand.OffsetBetweenCards))/2 + _hand.CardSize.CardScale.x / 2, i);
            ...
            //  for the different 
            if ((_hand.CardSize.CardScale.x + _hand.OffsetBetweenCards) * i - _hand.OffsetBetweenCards + _hand.LeftShift * 2 > _activeAreaSize)
            {
                Debug.Log("shit");
                //  ... - card - c/2 - card - c/2 - card - c/2 - ...
                _distanceBetweenCentersOfCards -= (_hand.CardSize.CardScale.x / 2 + _hand.OffsetBetweenCards) / (_hand.HandSize - _fullHandSize);
            }
        }
